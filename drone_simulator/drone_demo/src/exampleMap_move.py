...
class MoveSquareClass(object):
  ...
  def move_forward_drone(self, value):
    rospy.loginfo("Moving forward...")
    self._move_msg.linear.x = value
    self._move_msg.angular.z = 0.0
    self.publish_once_in_cmd_vel(self._move_msg)

  # function that makes drone move angle
  def move_angle_drone(self):
    rospy.loginfo("Angle movement...")
  ...
  def move_square(self):
    ...
    time.sleep(sideSeconds*3)
    self.stop_drone()
    time.sleep(1.0)
    self.move_angle_drone()
    time.sleep(angleSeconds)
