...
bool DroneDrive::init()
{
  ...
  drone_pose_ = 0.0;//tb3_pose_ = 0.0;
  prev_drone_pose_ = 0.0;//prev_tb3_pose_ = 0.0;

  // initialize publishers
  cmd_vel_pub_   = nh_.advertise<geometry_msgs::Twist>(cmd_vel_topic_name, 10);
  // initialize subscribers
  laser_scan_sub_  = nh_.subscribe("scan", 10, &DroneDrive::laserScanMsgCallBack, this);
  state_sub_ = nh_.subscribe("ground_truth/state", 10, &DroneDrive::odomMsgCallBack, this);
...
bool DroneDrive::controlLoop()
{
  ...
  switch(state_num)
  {
    ...
    case 1://TB3_DRIVE_FORWARD:
      ROS_INFO("state f");
      updatecommandVelocity(1.0, 0.0);
      state_num = 0;
      break;

    case 2://TB3_RIGHT_TURN:
      ROS_INFO("state r");
